...
function pageFromHash() {
  ...
  if (['season', 'podcast', 'history', 'awards', 'teams'].includes(parts[0])) return { page: parts[0] }
  return { page: 'home' }
}
...
function Nav(current) {
  ...
  return el('header', { class: 'topbar' }, el('div', { class: 'wrap nav' }, [
    el('button', { class: 'brand', onclick: () => go('') }, [el('span', {}, 'LEAGUE'), ' OF BUMS']),
    linksNav,
    menuBtn,
  ]))
}
...
function Season() {
  ...
  const matchupsForSeason = league.matchups.filter(m => m.season === league.season)
  const matchCards = matchupsForSeason.length
    ? el('div', { class: 'cards two' }, matchupsForSeason.map(m => {
        const tA = team(m.teamAId), tB = team(m.teamBId)
        const aWin = m.isComplete && (m.teamAScore ?? 0) > (m.teamBScore ?? 0)
        const bWin = m.isComplete && (m.teamBScore ?? 0) > (m.teamAScore ?? 0)
        return el('div', { class: 'match' }, [
          el('div', { class: 'matchrow' }, [
            ...
            el('span', { class: 'score' }, m.teamAScore ?? '—'),
          ]),
          el('div', { class: 'dash' }),
          el('div', { class: 'matchrow' }, [
            el('button', { class: bWin ? 'winner team-link' : 'team-link', onclick: () => tB && go(`teams/${tB.slug}`) }, tB?.teamName ?? 'TBD'),
            el('span', { class: 'score' }, m.teamBScore ?? '—'),
          ]),
          el('div', { class: 'upcoming' }, m.isComplete ? 'FINAL' : `WEEK ${m.week} · UPCOMING`),
        ])
      }))
    : Empty('No matchups scheduled yet.')
  ...
  return el('div', {}, [
    ...
    el('section', { class: 'section band' }, el('div', { class: 'wrap' }, [
      el('div', { class: 'sectionhead' }, el('h2', {}, 'HEAD-TO-HEAD MATCHUPS')),
      matchCards,
    ])),
    el('section', { class: 'section' }, el('div', { class: 'wrap' }, [
    ...
    el('section', { class: 'section' }, el('div', { class: 'wrap' }, el('div', { class: 'cards three' },
      league.teams.map(t =>
        el('button', { class: 'card team-card', onclick: () => go(`teams/${t.slug}`) }, [
          t.logoUrl ? el('img', { src: t.logoUrl, alt: '', class: 'team-logo' }) : el('span', { class: 'initials' }, t.teamName.split(' ').map(w => w[0]).slice(0, 2).join('')),
          el('h2', {}, t.teamName),
...
function TeamPage(slug) {
  const t = league.teams.find(x => x.slug === slug)
  if (!t) {
    return el('div', {}, [
      Hero('404', 'TEAM NOT FOUND', "That team isn't in the league data yet.", true),
      el('section', { class: 'section' }, el('div', { class: 'wrap' }, el('button', { class: 'btn', onclick: () => go('teams') }, '← BACK TO TEAMS'))),
    ])
  }
  const history = league.teamHistory.filter(h => h.teamId === t.id).sort((a, b) => b.season - a.season)
  ...
  const historyTable = history.length
    ...
    : Empty('No season records logged yet.')

  return el('div', {}, [
    el('section', { class: 'team-hero' }, el('div', { class: 'wrap' }, [
      el('button', { class: 'text-link', onclick: () => go('teams') }, '← ALL TEAMS'),
      el('div', { class: 'team-title' }, [
        el('div', { class: 'team-badge' }, t.logoUrl ? el('img', { src: t.logoUrl, alt: '' }) : t.teamName.split(' ').map(w => w[0]).slice(0, 2).join('')),

...
async function Season() {
  ...
  try {
    ...
    rankRows = order.length
      ? order.map((name, idx) => {
          const rank = idx + 1
          const t = teams.find(tm => tm.slug === slugify(name))
          const trend = pr.trends?.[name] || 'same'
          const defaultSub = t ? `${t.wins}-${t.losses}${t.ties ? `-${t.ties}` : ''} · ${t.pointsFor} PF · ${t.managerName}` : 'Team not found in Sleeper — check the spelling in data.js.'
          return el('div', { class: 'rank' }, [
            el('div', { class: 'ranknum' }, String(rank).padStart(2, '0')),
            el('div', {}, [
              el('button', { class: 'team-link', onclick: () => t && go(`teams/${t.slug}`) }, t?.teamName ?? name),
              el('div', { class: 'sub' }, pr.blurbs?.[name] || defaultSub),
            ]),
            el('div', { class: `trend ${trend}` }, trend === 'up' ? '▲' : trend === 'down' ? '▼' : '—'),
          ])
        })
      : [Empty('No power rankings posted yet.')]
  } catch (err) {
    rankRows = [SleeperError(err)]
  }
    ...
    el('section', { class: 'section' }, el('div', { class: 'wrap' }, [
      el('div', { class: 'sectionhead' }, [
        el('div', { class: 'sectionhead-title' }, [
          el('h2', {}, 'POWER RANKINGS'),
          el('span', { class: 'sponsor-tag' }, "SPONSORED BY D'S DIRTY DAWGS"),
        ]),
        el('span', { class: 'label' }, pr?.week ? `WEEK ${pr.week}` : 'NO WEEK POSTED'),
      ]),
      ...rankRows,
    ])),
...
async function History() {
  ...
  // Live trades, pulled straight from Sleeper's transaction log for this league.
  let liveTradesSection
  try {
    const [trades, teams] = await Promise.all([getSleeperTrades(), getSleeperTeams()])
    const teamsByRosterId = new Map(teams.map(t => [t.rosterId, t]))
    liveTradesSection = trades.length
      ? el('div', { class: 'articles topgap' }, trades.map(trade => {
          const dateLabel = trade.timestamp ? new Date(trade.timestamp).toLocaleDateString(undefined, { year: 'numeric', month: 'short', day: 'numeric' }) : 'DATE UNKNOWN'
          return el('article', { class: 'article' }, [
            el('div', { class: 'label' }, `${dateLabel}${trade.week ? ` · WEEK ${trade.week}` : ''}`),
            el('div', { class: 'trade-sides' }, trade.sides.map(side => {
              const t = teamsByRosterId.get(side.rosterId)
              const items = [...side.players, ...side.picks]
              return el('div', { class: 'trade-side' }, [
                el('button', { class: 'team-link', onclick: () => t && go(`teams/${t.slug}`) }, t?.teamName ?? `Roster ${side.rosterId}`),
                el('div', { class: 'sub' }, items.length ? `RECEIVES: ${items.join(', ')}` : 'RECEIVES: NOTHING LISTED'),
              ])
            })),
          ])
        }))
      : Empty('No completed trades found in Sleeper for this league yet.')
  } catch (err) {
    liveTradesSection = SleeperError(err)
  }

  // Manual archive for trades made before this Sleeper league existed (older seasons, etc).
  const archivedTradesSection = league.trades.length
  ...
  return el('div', {}, [
    ...
    el('section', { class: 'section band' }, el('div', { class: 'wrap' }, [
      el('h2', {}, 'EVERY TRADE EVER RECORDED'),
      liveTradesSection,
      archivedTradesSection ? el('div', { class: 'topgap' }, [
        el('div', { class: 'label topgap' }, 'ARCHIVED TRADES (PRE-SLEEPER)'),
        archivedTradesSection,
      ]) : null,
    ])),
...
async function render() {
  ...
  try {
    page =
      ...
      route.page === 'history' ? await History() :
